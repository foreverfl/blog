...
const Comment = ({}) => {
  ...
  const handleEditCommentChange = (commentId: string, value: string) => {
    setEditComment((prev) => ({ ...prev, [commentId]: value }));
  };

  // 관리자 댓글 핸들러
  const handleAdminReply = async (commentId: string) => {
    const adminComment = newAdminComments[commentId]?.trim();
    if (!adminComment) return;

    const res = await fetch("/api/comment/admin/add", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        pathHash,
    ...
    if (res.ok) {
      setNewAdminComments((prev) => ({ ...prev, [commentId]: "" })); // 해당 댓글의 관리자 댓글 초기화
      ...
      {Array.isArray(comments) && comments.length > 0 ? (
        comments.map((comment) => (
          <div key={comment._id} className="">
            ...
            <div className="flex items-start mb-5">
              ...
              <div className="flex flex-col">
                ...
                <div className="flex justify-end mt-2 space-x-2">
                  ...
                  {/* 답장 버튼 */}
                  {user?.email === "forevermfl@gmail.com" && (
                    <button onClick={() => toggleReplyMode(comment._id)}>
                      <svg
                        className="w-4 h-4 text-gray-800 dark:text-white"
                        aria-hidden="true"
                        xmlns="http://www.w3.org/2000/svg"
            ...
            {/* 관리자 대댓글 */}
            {!isReplying[comment._id] && comment.adminComment && (
            ...
            {isReplying[comment._id] && (
              <div className="ml-7 flex items-start my-4">
                <div className="relative flex flex-col w-full mx-3">
                  <textarea
                    className="w-full bg-blue-500 rounded-md text-lg dark:text-white p-2 resize-none"
                    rows={2}
                    value={newAdminComments[comment._id] || ""}
                    onChange={(e) =>
                      handleAdminCommentChange(comment._id, e.target.value)
                    }
                  />
                  <button
                    onClick={() => handleAdminReply(comment._id)}
                    className="absolute right-3 bottom-3 bg-transparent text-white py-1 px-4 rounded transition duration-300"
                  >
                    <svg
